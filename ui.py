...
def range_controls(
    ...
    with col1:
        rango = st.selectbox("Rango rápido", rr_options, index=rr_options.index(st.session_state[rr_key]),
    ...
    def _range_from_quick(sel: str):
        ...
        elif sel == "1 año": d_ini = max(dmin, today - dt.timedelta(days=365))
        elif sel == "YTD":   d_ini = dt.date(today.year, 1, 1)
        elif sel == "2 años": d_ini = max(dmin, today - dt.timedelta(days=365*2))
        elif sel == "Máximo": d_ini = dmin
        else: return None
        return d_ini, dmax
...
def kpi_quad(title: str, color: str, last_value: Optional[float], is_percent: bool,
    ...
    <div class="series-kpi">
      ...
      <div class="row4">
        <div class="cell"><div class="lbl">Último dato <span class="q" data-tip="{tip_last}">?</span></div>
        ...
        <div class="cell"><div class="lbl">Δ en el período <span class="q" data-tip="{tip_per}">?</span></div>
             <div class="val">{_fmt_pct(d_per)}</div></div>
      </div>
    </div>
    """, unsafe_allow_html=True)

# Compat para páginas viejas:
...
def series_picker(options: Sequence[str], default: Sequence[str] | None = None, *,
                  max_selections: int = 3, key: str = "series", title: str = "Elegí hasta 3 series",
                  subtitle: str | None = None, palette: Sequence[str] = ("#60A5FA", "#F87171", "#34D399", "#F59E0B", "#A78BFA"),
                  show_chips: bool = True) -> list[str]:
    placeholder = "Buscar / seleccionar…"; state_key = f"picker_{key}"
    ...
    with st.container():
        ...
        with col1:
            ...
            st.markdown(f"<div class='head'><div class='title'>🎛 {title} {pill}</div>{sub}</div>", unsafe_allow_html=True)
        with col2:
            if st.button("⟳ Limpiar", key=f"clear_{key}", use_container_width=True): st.session_state[state_key] = []
