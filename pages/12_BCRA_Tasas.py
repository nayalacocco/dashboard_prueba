...
# Sugeridos (ajustá keywords a tus descripciones reales)
...
if sel:
    ...
    if not dfw.empty:
        if freq.startswith("Mensual"):
            dfw = dfw.resample("M").mean()
        fig = px.line(dfw.reset_index(), x="fecha", y=sel, labels={"value":"% TNA","fecha":"Fecha","variable":"Tasa"})
        fig.update_layout(height=600, margin=dict(t=50,b=60), legend=dict(orientation="h", y=-0.2, x=0.5, xanchor="center"))
